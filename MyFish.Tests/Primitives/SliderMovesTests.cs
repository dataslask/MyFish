...
namespace MyFish.Tests.Primitives
{
    ...
    public class SliderMovesTests
    {
        ...
        {
            new SliderMoves<Pawn>("d3", TestBoard.With("pd3"), Vector.East).Join().Should().Be("e3 f3 g3 h3");
        }

        [Test]
        public void Should_move_west_correctly_on_empty_board()
        {
            new SliderMoves<Pawn>("d7", TestBoard.With("pd7"), Vector.West).Join().Should().Be("c7 b7 a7");
        } 
        
        [Test]
        public void Should_move_north_east_correctly_on_empty_board()
        {
            new SliderMoves<Pawn>("c5", TestBoard.With("pc5"), Vector.NorthEast).Join().Should().Be("d6 e7 f8");
        } 
        
        [Test]
        public void Should_move_south_west_correctly_on_empty_board()
        {
            new SliderMoves<Pawn>("g5", TestBoard.With("pg5"), Vector.SouthWest).Join().Should().Be("f4 e3 d2 c1");
        }

        [Test]
        public void Should_move_correctly_when_combining_vectors()
        {
            var moves = new SliderMoves<Pawn>("d3", TestBoard.With("pd3"), Vector.East, Vector.West, Vector.North, Vector.South);
        ...
        public void Should_stop_when_taking_opponent_pice()
        {
            ...
            new SliderMoves<Rook>("a1", board, Vector.North).Join().Should().Be("a2 a3 a4 a5 a6 a7");
        }
        ...
        public void Should_stop_in_front_of_friendly_pices()
        {
            ...
            new SliderMoves<Rook>("a1", board, Vector.North).Join().Should().Be("a2 a3 a4 a5 a6");
        }
